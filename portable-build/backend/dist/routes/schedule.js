...
preferencesRouter.get('/:key', (req, res) => {
    try {
        const item = getOne('SELECT value FROM user_preferences WHERE key = ?', [req.params.key]);
        if (!item) {
...
absenceLimitsRouter.post('/', (req, res) => {
    try {
        ...
        const existing = getOne('SELECT id FROM employee_absence_limits WHERE employeeId = ? AND absenceTypeId = ? AND year = ?', [employeeId, absenceTypeId, year]);
        if (existing) {
            runQuery(`
...
absenceLimitsRouter.post('/bulk', (req, res) => {
    try {
        ...
        for (const limit of limits) {
            const existing = getOne('SELECT id FROM employee_absence_limits WHERE employeeId = ? AND absenceTypeId = ? AND year = ?', [employeeId, limit.absenceTypeId, year]);
            if (existing) {
                runQuery(`
          UPDATE employee_absence_limits 
          SET totalDays = ?
...
absencesRouter.put('/:id', (req, res) => {
    try {
        ...
        const oldAbsence = getOne('SELECT * FROM absences WHERE id = ?', [req.params.id]);
        runQuery(`
      UPDATE absences 
...
absencesRouter.delete('/:id', (req, res) => {
    try {
        ...
        const absence = getOne('SELECT * FROM absences WHERE id = ?', [req.params.id]);
        if (absence) {
...
emailRouter.post('/send', async (req, res) => {
    try {
        ...
        const settingRow = getOne('SELECT value FROM settings WHERE key = ?', ['app-settings']);
        let smtpConfig = null;
        if (settingRow) {
