...
import { runQuery, getAll, getOne, getDatabaseBuffer, replaceDatabase } from '../database/db.js';
...
settingsRouter.get('/', (req, res) => {
    try {
        const setting = getOne('SELECT value FROM settings WHERE key = ?', ['app-settings']);
        if (!setting) {
            return res.json({
                language: 'en',
                darkMode: true,
    ...
    try {
        ...
        switch (moduleName) {
            ...
            case 'tests':
                ...
                break;
            case 'projects': {
                const allProjects = getAll('SELECT * FROM projects');
                data = {
...
dataRouter.post('/import', (req, res) => {
    try {
        const { customers, types, parts, tests, projects, settings, employees, scheduleAssignments, absenceTypes, absences, absenceLimits, employeeDetails, qualifications, holidays, comments, logs, templates, preferences } = req.body;
        // Clear existing data
      ...
      `, [
            ...
            if (p.weeks) {
                for (const [week, data] of Object.entries(p.weeks)) {
                    const weekData = data;
                    runQuery('INSERT INTO project_weeks (project_id, week, ist, soll, stoppage, production_lack) VALUES (?, ?, ?, ?, ?, ?)', [p.id, week, weekData.ist, weekData.soll, weekData.stoppage ? 1 : 0, weekData.productionLack ? 1 : 0]);
                }
            }
        });
        // Import employees
        if (employees?.length) {
            ...
            employees.forEach((e) => {
                runQuery(`INSERT INTO employees (id, firstName, lastName, color, status, suggestedShift, role, email, phone, department, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [e.id, e.firstName, e.lastName, e.color, e.status || 'available', e.suggestedShift, e.role || 'worker', e.email, e.phone, e.department, e.created_at || e.createdAt]);
            });
        }
        ...
        if (scheduleAssignments?.length) {
            ...
            scheduleAssignments.forEach((a) => {
                runQuery(`INSERT INTO schedule_assignments (id, employeeId, projectId, testId, week, shift, scope, note, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [a.id, a.employeeId, a.projectId, a.testId, a.week, a.shift || 1, a.scope || 'project', a.note, a.created_at || a.createdAt]);
            });
        }
        ...
        if (absenceTypes?.length) {
            ...
            absenceTypes.forEach((t) => {
                runQuery(`INSERT INTO absence_types (id, name, icon, color, defaultDays, isPaid, requiresApproval, isActive, sortOrder) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [t.id, t.name, t.icon, t.color, t.defaultDays || 0, t.isPaid ?? 1, t.requiresApproval ?? 1, t.isActive ?? 1, t.sortOrder || 0]);
            });
        }
        // Import absences
        if (absences?.length) {
            ...
            absences.forEach((a) => {
                runQuery(`INSERT INTO absences (id, employeeId, absenceTypeId, startDate, endDate, workDays, status, note, createdAt, approvedAt, approvedBy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [a.id, a.employeeId, a.absenceTypeId, a.startDate, a.endDate, a.workDays || 1, a.status || 'approved', a.note, a.createdAt, a.approvedAt, a.approvedBy]);
            });
        }
        // Import absence limits
        if (absenceLimits?.length) {
            runQuery('DELETE FROM employee_absence_limits', []);
            absenceLimits.forEach((l) => {
                runQuery(`INSERT INTO employee_absence_limits (id, employeeId, absenceTypeId, year, totalDays, usedDays) VALUES (?, ?, ?, ?, ?, ?)`, [l.id, l.employeeId, l.absenceTypeId, l.year, l.totalDays || 0, l.usedDays || 0]);
            });
        }
        // Import employee details
        if (employeeDetails?.length) {
            runQuery('DELETE FROM employee_details', []);
            employeeDetails.forEach((d) => {
                runQuery(`INSERT INTO employee_details (employeeId, email, phone, birthDate, hireDate, department, position, contractType, workingHours, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [d.employeeId, d.email, d.phone, d.birthDate, d.hireDate, d.department, d.position, d.contractType, d.workingHours || 40, d.notes]);
            });
        }
        ...
        if (qualifications?.length) {
            ...
            qualifications.forEach((q) => {
                runQuery(`INSERT INTO employee_qualifications (id, employeeId, testId, level, certifiedAt, expiresAt) VALUES (?, ?, ?, ?, ?, ?)`, [q.id, q.employeeId, q.testId, q.level || 1, q.certifiedAt, q.expiresAt]);
            });
        }
        // Import holidays
        if (holidays?.length) {
            ...
            holidays.forEach((h) => {
                runQuery(`INSERT INTO holidays (id, date, name, isMovable) VALUES (?, ?, ?, ?)`, [h.id || h.date, h.date, h.name, h.isMovable || 0]);
            });
        }
        // Import comments
        if (comments?.length) {
            ...
            comments.forEach((c) => {
                runQuery(`INSERT INTO comments (id, projectId, week, text, createdAt) VALUES (?, ?, ?, ?, ?)`, [c.id, c.projectId, c.week, c.text, c.createdAt]);
            });
        }
        // Import logs
        if (logs?.length) {
            ...
            logs.forEach((l) => {
                runQuery(`INSERT INTO logs (id, userId, userName, action, entityType, entityName, details, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [l.id, l.userId, l.userName, l.action, l.entityType, l.entityName, l.details, l.timestamp]);
            });
        }
        // Import templates
        if (templates?.length) {
            ...
            templates.forEach((t) => {
                runQuery(`INSERT INTO schedule_templates (id, name, data, createdAt) VALUES (?, ?, ?, ?)`, [t.id, t.name, t.data, t.createdAt]);
            });
        }
        // Import settings
        if (settings) {
        ...
        if (preferences?.length) {
            ...
            preferences.forEach((p) => {
                runQuery(`INSERT INTO user_preferences (key, value) VALUES (?, ?)`, [p.key, p.value]);
            });
        }
        res.json({ success: true });
    }
        ...
        switch (moduleName) {
            case 'planning': {
                ...
                data.projects?.forEach((p) => {
                    runQuery(`INSERT INTO projects (id, customer_id, type_id, part_id, test_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`, [p.id, p.customer_id || p.customerId, p.type_id || p.typeId, p.part_id || p.partId, p.test_id || p.testId, p.created_at || p.createdAt, p.updated_at || p.updatedAt]);
                    if (p.weeks) {
                        for (const [week, weekData] of Object.entries(p.weeks)) {
                            const wd = weekData;
                            runQuery('INSERT INTO project_weeks (project_id, week, ist, soll, stoppage, production_lack) VALUES (?, ?, ?, ?, ?, ?)', [p.id, week, wd.ist, wd.soll, wd.stoppage ? 1 : 0, wd.productionLack ? 1 : 0]);
                        }
                    }
                });
            ...
            case 'employees': {
                ...
                data.employeeDetails?.forEach((d) => {
                    runQuery(`INSERT INTO employee_details (employeeId, email, phone, birthDate, hireDate, department, position, contractType, workingHours, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [d.employeeId, d.email, d.phone, d.birthDate, d.hireDate, d.department, d.position, d.contractType, d.workingHours || 40, d.notes]);
                });
            ...
            case 'schedule': {
                runQuery('DELETE FROM schedule_assignments', []);
                runQuery('DELETE FROM schedule_templates', []);
                data.scheduleAssignments?.forEach((a) => {
                    runQuery(`INSERT INTO schedule_assignments (id, employeeId, projectId, testId, week, shift, scope, note, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [a.id, a.employeeId, a.projectId, a.testId, a.week, a.shift || 1, a.scope || 'project', a.note, a.created_at || a.createdAt]);
                });
            ...
            case 'absences': {
                ...
                runQuery('DELETE FROM absence_types', []);
                runQuery('DELETE FROM holidays', []);
                data.absenceTypes?.forEach((t) => {
                    runQuery(`INSERT INTO absence_types (id, name, icon, color, defaultDays, isPaid, requiresApproval, isActive, sortOrder) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [t.id, t.name, t.icon, t.color, t.defaultDays || 0, t.isPaid ?? 1, t.requiresApproval ?? 1, t.isActive ?? 1, t.sortOrder || 0]);
                });
                data.absences?.forEach((a) => {
                    runQuery(`INSERT INTO absences (id, employeeId, absenceTypeId, startDate, endDate, workDays, status, note, createdAt, approvedAt, approvedBy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [a.id, a.employeeId, a.absenceTypeId, a.startDate, a.endDate, a.workDays || 1, a.status || 'approved', a.note, a.createdAt, a.approvedAt, a.approvedBy]);
                });
                data.absenceLimits?.forEach((l) => {
                    runQuery(`INSERT INTO employee_absence_limits (id, employeeId, absenceTypeId, year, totalDays, usedDays) VALUES (?, ?, ?, ?, ?, ?)`, [l.id, l.employeeId, l.absenceTypeId, l.year, l.totalDays || 0, l.usedDays || 0]);
                });
...
dataRouter.get('/backups', (req, res) => {
    try {
        const backupPath = req.query.path || join(__dirname_routes, '../../../backups');
        if (!existsSync(backupPath)) {
            return res.json({ backups: [], path: backupPath });
        }
        const files = readdirSync(backupPath)
            .filter((f) => f.startsWith('kappa-backup-') && f.endsWith('.json'))
            .map((f) => {
